...
export const SKILLS: Skill[] = [
  ...
  { id: 'redis', name: 'Redis (async)', cat: 'python', level: 'proficient', start: 2024, end: null,
    desc: 'Cache, debounce patterns, pipeline operations via redis.asyncio. State coordination between multi-agent workers.',
    context: 'Used in Clivia for chatbot debounce and multi-agent state control, and in Count on Sheep\'s STT pipeline to reduce redundant LLM calls during live consultations.',
    projects: ['Clivia', 'Count on Sheep'] },
  /* AI / LLM */
  ...
  { id: 'mcp', name: 'MCP (Model Context Protocol)', cat: 'ai', level: 'proficient', start: 2025, end: null,
    desc: 'Custom MCP client (lazy server discovery, tool routing, stdio/SSE/streamable-http) and FastMCP server.',
    context: 'Adopted MCP at launch (Nov 2024) — already had production multi-agent experience with LangGraph, which made the protocol\'s tool-routing model immediately intuitive. Built Count on Sheep\'s internal MCP client from scratch, letting AI agents safely access internal tools without exposing raw APIs.',
    projects: ['Count on Sheep'] },
  ...
  { id: 'traefik', name: 'Traefik', cat: 'devops', level: 'proficient', start: 2024, end: null,
    desc: 'Reverse proxy with automatic TLS, routing rules, middleware configuration. Always deployed alongside Portainer.',
    context: 'First used at Clivia to route Docker Swarm services, then at Count on Sheep — eliminated manual SSL renewals and enabled clean subdomain routing for 8+ internal services on a single VPS.',
    projects: ['Clivia', 'Count on Sheep infra'] },
  { id: 'portainer', name: 'Portainer', cat: 'devops', level: 'proficient', start: 2024, end: null,
    desc: 'Web UI for Docker and Docker Swarm management — container lifecycle, logs, volumes, networks, and webhook-triggered GitOps deploys. Always paired with Traefik.',
    context: 'Used at Clivia to manage a Docker Swarm cluster and at Count on Sheep for VPS container orchestration — webhook deploys triggered by git push replaced manual SSH-based workflows entirely.',
    projects: ['Clivia', 'Count on Sheep infra'] },
